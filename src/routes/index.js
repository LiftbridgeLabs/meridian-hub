...
router.get('/hub/settings', requireAuth, (req, res) => {
  const settings = db
    .prepare('SELECT id, setup_complete, license_key, max_households, created_at FROM hub_settings WHERE id = 1')
    .get();
  res.json(settings);
});

router.get('/hub/stats', requireAuth, (req, res) => {
  const settings = db.prepare('SELECT max_households, created_at FROM hub_settings WHERE id = 1').get();
